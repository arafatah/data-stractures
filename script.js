...
// 2. Use a loop to calculate the average odd and log it to the console (already studied how to calculate averages, go check if you don't remember)
const {
  odds: { team1, x: draw, team2 },
} = game;
// console.log(team1, draw, team2);
...
const calcOdds = function (t1, d, t2) {
  const val = [t1, t2, d];
  const avrOdds = (t1 + t2 + d) / val.length;
  console.log(avrOdds);
  return avrOdds;
};
calcOdds(team1, team2, draw);

// Second way. Esiest way.
const odds = Object.values(game.odds);
let average = 0;
...
rest.set(1, 'Friedst');
console.log(rest.set(2, 'listbon'));

rest
  .set('category', ['Italian', 'Pizzabiar', 'varigabel', 'Organic'])
  .set('open', 11)
  .set('close', 22)
